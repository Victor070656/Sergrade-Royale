...
if (isset($_GET["id"])) {
    $id = $_GET["id"];
    $getBlogs = $conn->query("SELECT * FROM `blogs` WHERE `id` = '$id'");
    if ($getBlogs->num_rows < 1) {
        echo "<script>location.href = 'blogs.php';</script>";
    }
    $blogs = $getBlogs->fetch_assoc();
} else {
    echo "<script>location.href = 'blogs.php';</script>";
}
...
<head>
    ...
    <!-- Title -->
    <title>Sergrade Admin || Edit Blog</title>
</head>
                                                ...
                                                <div class="row row-gap-3">
                                                    ...
                                                    <div class="col-12">
                                                        <div class="">
                                                            ...
                                                            <input type="text" class="form-control" id="title" value="<?= $blogs['title']; ?>" name="title" required>
                                                        </div>
                                                    </div>
                                                    <div class="col-12">
                                                        <div class="">
                                                            <label for="password" class="form-label">Body</label> <span class="text-danger">*</span>
                                                            <textarea class="form-control" id="password" name="body" rows="9" required><?= $blogs['body']; ?></textarea>
                                                        </div>
                                                    </div>
                                                ...
                                                if (isset($_POST["update"])) {
                                                    ...
                                                    if (!empty($_FILES["image"]["name"])) {
                                                        if (move_uploaded_file($tmp_name, $path)) {
                                                            $add = $conn->query("UPDATE `blogs` SET `title` = '$title', `body` = '$body', `image` = '$image' WHERE `id` = '$id'");
                                                            if ($add) {
                                                                echo "<script>alert('Blog updated successfully'); location.href = 'blogs.php'</script>";
                                                            } else {
                                                        ...
                                                        $add = $conn->query("UPDATE `blogs` SET `title` = '$title', `body` = '$body' WHERE `id` = '$id'");
                                                        if ($add) {
                                                            echo "<script>alert('Blog updated successfully'); location.href = 'blogs.php'</script>";
                                                        } else {
                                                            echo "<script>alert('An error occured')</script>";
